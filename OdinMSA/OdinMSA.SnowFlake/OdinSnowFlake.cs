...
public class OdinSnowFlake : IOdinSnowFlake
{
    // 开始时间截((new DateTime(2020, 1, 1, 0, 0, 0, DateTimeKind.Utc)-Jan1st1970).TotalMilliseconds)
        ...
        // 数据中心ID(0~31)
        private long LogDatacenterId { get; set; }
        // 工作机器ID(0~31)
        private long LogWorkerId { get; set; }
        // 毫秒内序列(0~4095)
        ...
        public OdinSnowFlake(SnowFlakeModel model)
        {
        ...
        public string AnalyzeId(long Id)
        {
            ...
            var workerId = (Id ^ ((timestamp << TimestampLeftShift) | (LogDatacenterId << DatacenterIdShift))) >> WorkerIdShift;
